...
class _UploadScreenState extends State<UploadScreen> {
  ...
  final VideoSocketService _socketService = VideoSocketService();
  ...
  void _updateProgress() {
    setState(() {
      ...
      if (_videoService.videos.isNotEmpty) {
        final video = _videoService.videos.last;
        video.eventClipPaths = List.from(_socketService.clips);
      }
    });

    if (_progress >= 1.0 && _socketService.clips.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("${_socketService.clips.length} event clips generated!")),
      );
    }
  }
  ...
  void _extractEvents() {
    ...
    final video = _videoService.videos.last;
    if (video.path == null || video.path!.isEmpty) return;

    // Reset previous progress & clips
                  ...
                  children: [
                    ...
                    Column(
                      children: _videoService.videos.map((video) {
                        ...
                        if (video.path != null) outputs.add({'name': 'Original', 'path': video.path, 'color': Colors.blue});
                        if (video.clipPath != null) outputs.add({'name': 'Clip', 'path': video.clipPath, 'color': Colors.orange});
                        if (video.reelPath != null) outputs.add({'name': 'Reel', 'path': video.reelPath, 'color': Colors.green});
                        if (video.resolutionPaths != null) {
                          for (var r in video.resolutionPaths!) {
                            outputs.add({'name': 'Resolution ${r.split('_').last}', 'path': r, 'color': Colors.purple});
                          }
                        }
                        if (video.eventClipPaths != null) {
                          for (var i = 0; i < video.eventClipPaths!.length; i++) {
                            outputs.add({'name': 'Event Clip ${i + 1}', 'path': video.eventClipPaths![i], 'color': Colors.redAccent});
                          }
                        }
                        ...
                        return Card(
                          ...
                          child: ExpansionTile(
                            title: Text(video.name, style: const TextStyle(fontWeight: FontWeight.bold)),
                            children: outputs.map((o) {
                              return Container(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  border: Border.all(color: o['color'], width: 1),
                                ),
                                child: Row(
                                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                  children: [
                                    Expanded(
                                      child: Text(o['name'], style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.black87)),
                                    ),
                                    Row(
                                      children: [
                                        IconButton(
                                          icon: Icon(Icons.play_arrow, color: o['color']),
                                          onPressed: () => _startPreview(path: o['path'], name: "${video.name} - ${o['name']}"),
                                        ),
                                        IconButton(
                                          icon: const Icon(Icons.download, color: Colors.indigo),
                                          onPressed: () => _downloadFile(o['path'], "${o['name']}_${video.name}"),
                                        ),
                                      ],
                                    ),
                                  ],
                    ...
                    const SizedBox(height: 16),
                    ElevatedButton.icon(
                      style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)), backgroundColor: Colors.amberAccent),
                        ...
                        children: [
                          Stack(
                            ...
                            children: [
                              ...
                              Text("${(_progress * 100).toStringAsFixed(0)}%", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.white)),
                            ],
                          ),
                          const SizedBox(height: 8),
                        ],
